...
def bands_to_arrays(omega, bands, vals, deltas):
    ...
    """
    m = len(omega)
    v = np.zeros(m, dtype=np.complex64)
    d = np.ones(m)
...
class DesignPaulynomials(sp.app.App):
    ...
    def __init__(self, n, omega, m_xy, d_xy, m_z, d_z, beta, d_beta,
        ...
        # Get variables
        self.X = np.zeros((2 * n + 1, 2 * n + 1), dtype=np.complex64)
        self.X[0, 0] = 1
        self.a = self.X[1:n + 1, 0]
        self.b = self.X[n + 1:, 0]
        u = np.zeros(A.oshape, dtype=np.complex64)
...
def design_paulynomials(n, omega, m_xy, d_xy, m_z, d_z, beta, d_beta,
    ...
    # Trace equals to 1
    dirac = np.zeros(2 * n - 1, dtype=np.complex64)
    dirac[n - 1] = 1
